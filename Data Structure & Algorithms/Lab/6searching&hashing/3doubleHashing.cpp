// Lab 6.3: Implementation of double hashing with rehashing
#include <iostream>
#include <iomanip>
#include <math.h>
#include <stdlib.h>
...
public:
	...
	void insert(int key)
	{
		...
		else
		{
			...
			if (i < size)
			{
				if (i > 0)
					cout << endl
						 << "Collision occurred at index " << oldIndex << endl
						 << "After collision resolution using double hashing: " << endl;
				hashTable[finalIndex] = key;
				count++;
				cout << endl
					 << "Inserted " << key << " at index " << finalIndex << endl;
			}
			else
				cout << endl
					 << "Couldn't insert. Rehash may be needed." << endl;
		}
	}
	int search(int key)
	{
	...
	void rehash()
	{
		...
		delete[] oldTable;
		cout << endl
			 << "Rehashing complete.\nNew table size : " << size << endl
			 << "New prime : " << prime << endl;
	}
};
int main()
{
